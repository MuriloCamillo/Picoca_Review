...
function displaySeriesInfo() {
  ...
  if (!seriesId) {
    ...
    document.querySelector("main").innerHTML =
      '<h1 class="text-center text-danger mt-5">Erro: ID da série não especificado.</h1>';
    return;
  }
  ...
  if (!serie) {
    ...
    document.querySelector(
      "main"
    ).innerHTML = `<h1 class="text-center text-danger mt-5">Série com ID "${seriesId}" não encontrada!</h1>`;
    return;
  }

  // Preencher meta description
  const description = `${serie.title}: ${
    serie.synopsis ? serie.synopsis.substring(0, 160) + "..." : ""
  }`; // Adicionado verificação para serie.synopsis
  document
  ...
  const synopsisElement = document
    .getElementById("serie-synopsis")
    ?.querySelector("p"); // Pega o <p> dentro da div
  const statusElement = document.getElementById("serie-status");

  if (creatorElement)
    creatorElement.textContent = `Criado por ${serie.creator}`;
  ...
  if (backdropElement && serie.backdropImg)
    backdropElement.style.backgroundImage = `url('${serie.backdropImg}')`;
