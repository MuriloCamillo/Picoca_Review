...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (typeof seriesData === "undefined") {
    console.error(
      "Erro Crítico: objeto seriesData não encontrado. Verifique se assets/js/series_data.js foi carregado ANTES de navbar_search.js."
    );
  ...
  if (!searchInput || !searchResultsContainer || !searchForm) {
    console.warn(
      "Aviso: Elementos da busca na navbar (input, results container ou form) não encontrados nesta página."
    );
    return;
  }
  ...
  searchForm.addEventListener("submit", (event) => {
    ...
    if (searchTerm) {
      window.location.href = `series_gallery.html?search=${encodeURIComponent(
        searchTerm
      )}`;
    }
    searchResultsContainer.style.display = "none";
