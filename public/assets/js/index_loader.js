...
function createIndexNewsCardHTML(newsItem, newsId) {
  ...
  const shortSummary =
    summary.length > 90 ? summary.substring(0, 87) + "..." : summary;
...
function loadIndexNews() {
  ...
  if (typeof newsData === "undefined" || Object.keys(newsData).length === 0) {
    ...
    container.innerHTML =
      '<p class="text-light text-center col-12">Nenhuma notícia disponível.</p>';
    return;
  }
...
function createIndexSeriesPosterHTML(seriesId, series) {
  ...
  const link = `/series_info_default?id=${seriesId}`;
...
function loadIndexSeriesGalleries() {
  ...
  if (!trendsContainer || !rankedContainer) {
    console.warn(
      "Containers de galeria ('trends-gallery' ou 'ranked-gallery') não encontrados no index.html."
    );
    return;
  }
  ...
  if (
    ...
    console.warn(
      "Dados de séries (seriesData) não encontrados ou vazios para o index."
    );
    trendsContainer.innerHTML =
      '<p class="text-light">Erro ao carregar séries.</p>';
  ...
  if (trendsSeries.length > 0) {
    ...
    trendsHTML =
      '<p class="text-light">Nenhuma série em tendência no momento.</p>';
  }
