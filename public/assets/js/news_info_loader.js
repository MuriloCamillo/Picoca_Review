            ...
            <div class="card bg-dark text-light h-100">
                <a href="news_default.html?id=${newsId}">
                    <img src="${
                      ...
                      "assets/img/placeholder_news.webp"
                    }" class="card-img-top" alt="${
    newsItem.title || "Notícia"
  }">
                </a>
...
function displayRelatedNews(currentNewsId) {
  ...
  if (otherNewsIds.length < 1) {
    relatedNewsContainer.innerHTML =
      '<p class="text-light text-center col-12">Não há outras notícias disponíveis.</p>';
    return;
  }
...
function displayNewsInfo() {
  ...
  if (!newsId) {
    ...
    document.querySelector("main").innerHTML =
      '<h1 class="text-center text-danger mt-5">Erro: ID da notícia não especificado.</h1>';
    return;
  }
  ...
  if (!news) {
    ...
    document.querySelector(
      "main"
    ).innerHTML = `<h1 class="text-center text-danger mt-5">Notícia com ID "${newsId}" não encontrada!</h1>`;
    return;
  }
  ...
  const description =
    news.summary ||
    (news.articleContent ? news.articleContent.substring(0, 160) + "..." : ""); // Adicionado verificação para news.articleContent
  document
  ...
  if (titleElement)
    titleElement.textContent = news.title || "Título Indisponível";

  const authorDateElement = document.getElementById("news-author-date");
  if (authorDateElement)
    authorDateElement.textContent = `Por ${
      news.author || "Autor Desconhecido"
    } - ${news.date || "Data Indisponível"}`;
  ...
  if (mainCaptionElement)
    mainCaptionElement.textContent = news.mainImageCaption || "";
  ...
  const secondaryContentElement = document.getElementById(
    ...
    if (news.videoUrl) {
      secondaryContentElement.innerHTML = `
            <div class="text-center my-4">
                <div class="ratio ratio-16x9 mx-auto" style="max-width: 640px;">
                    <iframe src="${
                      news.videoUrl
                    }" title="Vídeo da Notícia" allowfullscreen class="rounded"></iframe>
                </div>
            </div>
            ${
              news.videoCaption
                ? `<p class="img-leg">${news.videoCaption}</p>`
                : ""
            }
        `;
      ...
      secondaryContentElement.innerHTML = `
            <div class="text-center my-4">
              ...
              }" alt="Imagem secundária da notícia" class="img-fluid rounded">
            </div>
            ${
              news.secondaryImageCaption
                ? `<p class="img-leg">${news.secondaryImageCaption}</p>`
                : ""
            }
        `;
    }
